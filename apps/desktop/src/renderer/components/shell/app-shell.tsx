...
type SidebarWidthController = ReturnType<typeof usePersistentSidebarWidth>;
...
function PrimarySidebar({
  ...
  controller: SidebarWidthController;
  destination: ShellController["destination"];
  ...
  return (
    <aside
      className={cn(
        ...
        !controller.resizing && "transition-[width] duration-200 ease-in-out",
        !open && "border-r-transparent",
      )}
      style={{ width: open ? controller.width : 0 }}
      aria-hidden={!open}
      inert={interactionLocked || !open}
    >
      ...
      <div className="flex min-w-[220px] items-center gap-1 p-2">
        <AccountMenu width={controller.width} />
        <Tooltip>
          <TooltipTrigger
            render={
              <Button
...
function ShellHeader({
  ...
  interactionLocked: boolean;
  isMac: boolean;
  projectSection: ShellController["projectSection"];
  ...
  return (
    <header
      ...
      inert={interactionLocked || undefined}
    >
      <SidebarToggle open={sidebarOpen} isMac={isMac} onToggle={onToggleSidebar} />
      <ProjectPanelToggles
        isMac={isMac}
        projectSection={projectSection}
        projectVisible={projectVisible}
        sidebarOpen={sidebarOpen}
      />
      <HeaderLocation destination={destination} projectVisible={projectVisible} title={title} />
      <HeaderActions projectVisible={projectVisible} />
    </header>
  );
}
...
function SidebarToggle({
  ...
  return (
    <Tooltip>
        ...
        render={
          <Button
            className={cn(
              ...
              open ? "left-2" : isMac ? "left-[76px]" : "left-2",
            )}
            size="icon"
            variant="ghost"
      ...
      <TooltipContent>
        {open ? "Collapse" : "Open"} sidebar ({isMac ? "⌘B" : "Ctrl+B"})
      </TooltipContent>
    </Tooltip>
  );
}

function ProjectPanelToggles({
  isMac,
  projectSection,
  projectVisible,
  sidebarOpen,
...
function HeaderActions({ projectVisible }: { projectVisible: boolean }) {
  return (
...
function AuxiliarySidebar({
  ...
  controller: SidebarWidthController;
  mode: ShellController["auxiliaryMode"];
  session: ShellController["session"];
  onConfigure: () => void;
}) {
  ...
  return (
    <aside
      ...
      style={{ width: mode ? controller.width : 0 }}
      aria-hidden={closed}
      inert={closed}
    >
      <div className="min-w-[260px] min-h-0 flex-1 overflow-y-auto">
        {mode === "agents" && session && (
          <AgentsSidebar key={session.directory} session={session} onConfigure={onConfigure} />
        )}
        {mode === "metrics" && <MetricsSidebar />}
      </div>
      <div
