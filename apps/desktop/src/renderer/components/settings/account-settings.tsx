...
type AccountBusy = "email" | "google" | "sign-out" | "refresh" | null;
...
function AccountCard({
  ...
  if (account.status === "signed-in" && account.user)
    return <SignedInCard user={account.user} busy={busy} onSignOut={onSignOut} />;
  if (!account.serviceAvailable)
    return <UnavailableCard account={account} busy={busy} onRefresh={onRefresh} />;
  return <SignInCard account={account} busy={busy} onSignIn={onSignIn} />;
}
  ...
  return (
    <div className="rounded-xl border border-border bg-panel p-6">
      ...
      <p className="mt-1 max-w-md text-ui leading-5 text-muted">
        Local editing still works normally. Try again when the authentication service is available.
      </p>
      {account.detail && (
