...
import { TranscriptChunkRunner } from "./chunk-runner";
import type { ActiveTranscriptJob } from "./chunk-runner";
...
export class TranscriptJobCoordinator {
  ...
  readonly #chunkRunner: TranscriptChunkRunner;
  ...
  constructor(
    ...
    this.#chunkRunner = new TranscriptChunkRunner(
  ...
  async setProject(directory: string, project: Project, scope: DerivedProjectScope): Promise<void> {
    return this.#serialize(async () => {
      await this.#stopAllJobs(this.#directory);
  ...
  async #stopAllJobs(directory: string | null): Promise<void> {
  ...
  async #requestJobsNow(scope: DerivedProjectScope, assetIds: readonly string[]) {
    ...
    for (const assetId of new Set(assetIds)) await this.#queueAsset(assetId);
    await this.#persistIndex();
    this.#emit();
    log.info(
      {
        ...
        queuedAssets: Object.values(this.#index.assets).filter(({ state }) => state === "queued")
          .length,
      },
  ...
  async cancelJobs(scope: DerivedProjectScope, assetIds: readonly string[]) {
    return this.#serialize(() => this.#cancelJobsNow(scope, assetIds));
  }

  async #cancelJobsNow(scope: DerivedProjectScope, assetIds: readonly string[]) {
    this.#assertScope(scope);
    if (assetIds.length === 0 || assetIds.length > MAX_REQUEST_IDS)
      throw new Error(`Select between 1 and ${MAX_REQUEST_IDS} transcript jobs to cancel`);
    for (const assetId of new Set(assetIds)) await this.#cancelAsset(assetId);
    await this.#persistIndex();
    this.#emit();
    return this.#snapshot(scope);
  }

  async #cancelAsset(assetId: string): Promise<void> {
    const asset = this.#requireAsset(assetId);
    ...
    const current = this.#index.assets[asset.id];
    if (!active && current?.state !== "queued") return;
  ...
  async beginJob(scope: DerivedProjectScope, assetId: string): Promise<{ jobId: string }> {
    return this.#serialize(() => this.#beginJobNow(scope, assetId));
  }

  async #beginJobNow(scope: DerivedProjectScope, assetId: string): Promise<{ jobId: string }> {
    this.#assertScope(scope);
    this.#requireTranscriptionService();
    if (this.#jobs.size >= MAX_ACTIVE_JOBS) throw new Error("Too many active transcription jobs");
    const asset = this.#requireAsset(assetId);
    if (this.#index.assets[asset.id]?.state !== "queued")
      throw new Error("Transcript job is not queued");
  ...
  async transcribeChunk(
    ...
    this.#assertScope(scope);
    const gateway = this.#requireTranscriptionService();
    await this.#chunkRunner.run(this.#requireJob(input.jobId), input, gateway);
  }

  async finalizeJob(scope: DerivedProjectScope, jobId: string): Promise<TranscriptSnapshot> {
    return this.#serialize(() => this.#finalizeJobNow(scope, jobId));
  }

  async #finalizeJobNow(scope: DerivedProjectScope, jobId: string): Promise<TranscriptSnapshot> {
    this.#assertScope(scope);
    const job = this.#requireJob(jobId);
    this.#validateFinalizable(job);
    job.state = "finalizing";
    try {
      const artifact = await this.#stitchJob(job);
      await this.#publishArtifact(job, artifact);
      this.#logCompletedJob(job, artifact);
  ...
  #validateFinalizable(job: ActiveTranscriptJob): void {
  ...
  #stitchJob(job: ActiveTranscriptJob): Promise<TranscriptArtifact> {
    const directory = this.#requireDirectory();
    return stitchTranscriptChunks({
      asset: job.asset,
  ...
  async #publishArtifact(job: ActiveTranscriptJob, artifact: TranscriptArtifact): Promise<void> {
  ...
  #logCompletedJob(job: ActiveTranscriptJob, artifact: TranscriptArtifact): void {
    log.info(
      {
  ...
  #requireTranscriptionService(): Pick<TranscriptionGateway, "transcribe"> {
