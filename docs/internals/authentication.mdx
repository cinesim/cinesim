...
description: How optional accounts, sessions, and email verification work without entering canonical project state.
---

Accounts are optional and exist only to unlock cloud projects. This page covers how that system is
built and how to run it locally. Nothing here touches canonical project state.
...
3. Run `vp run api:generate`.
4. Review and commit the SQL migration and Drizzle metadata.
5. Apply it locally with `vp run api:setup`.
