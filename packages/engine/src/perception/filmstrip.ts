...
export function filmstripSampleTimes(
  ...
  if (count === 1) return [timeUs(Math.round((durationUs - 1) / 2))];
  return Array.from({ length: count }, (_, index) =>
    timeUs(Math.round((index / (count - 1)) * Math.max(0, durationUs - 1))),
  );
}
...
export function pointerSourceTimeUs(
  ...
  return timeUs(Math.round(ratio * Math.max(0, durationUs - 1)));
}
